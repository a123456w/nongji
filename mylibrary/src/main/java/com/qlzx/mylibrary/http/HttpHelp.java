...
import com.qlzx.mylibrary.BuildConfig;
...
public class HttpHelp {
    ...
    public static Retrofit getInstance(){
        if (instance==null){
            synchronized (HttpHelp.class){
                if (instance==null){
                    ...
                    instance = new Retrofit.Builder()
                            .baseUrl(Constants.type?BuildConfig.Base_Url:BuildConfig.CBase_Url)
                            .client(client)
                            .addCallAdapterFactory(RxJavaCallAdapterFactory.create())
                            .addConverterFactory(GsonConverterFactory.create(gson))
                            .build();
            ...
            String url = Constants.type?BuildConfig.Base_Url:BuildConfig.CBase_Url;
            if(!instance.baseUrl().toString().equals(url)){
                instance = null;
                HttpLoggingInterceptor interceptor = new HttpLoggingInterceptor();
                interceptor.setLevel(HttpLoggingInterceptor.Level.BODY);
                OkHttpClient client = new OkHttpClient.Builder()
