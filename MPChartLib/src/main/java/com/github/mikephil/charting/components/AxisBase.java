...
public abstract class AxisBase extends ComponentBase {
     ...
     * 当为true时，轴标记由“粒度”属性控制。当假时，轴值可能被重复.
     * 如果两个相邻的轴值被舍入到相同的值，则可能发生这种情况。
     * 如果使用粒度，这可以避免通过较少的轴值可见。
     */
    protected boolean mGranularityEnabled = false;

    /**
     * if true, the set number of y-labels will be forced
     * 如果设置为true，该集数Y将被强制标签
     */
     ...
     *
     * 中心轴标签，而不是在他们原来的位置绘制它们。这对于分组巴氏图特别有用。
     *
     * @param enabled
     */
     ...
     * 将此设置为true以启用绘制该轴的标签（这将不会）影响绘制网格线或轴线。
     *
     * @param enabled
     */
     ...
     *     设置y轴max＝25，min＝2，默认值为6的标签条目的数量，注意这个数字不是固定的。
     *
...
//            count = 25;
     ...
     *  设置x轴max=25，min=2的标签条目的数量，默认值：6，注意这个数目不是固定的（如果.==false），只能近似。
     *
     * @param count the number of y-axis labels that sould be displayed
    ...
    /**
     * @return true if granularity is enabled 如果启用粒度
     */
     ...
     * 为该轴设置自定义最小值。如果设置，此值将不计算。
     * 自动依赖
     * 提供的数据。使用ReStAxIsMimalValue（）来撤消此操作。别忘了打电话
     * 如果使用，设置STATSTATOL0（false）
     * 此方法。否则，轴的最小值仍将被强制为0。
     *
     * @param min
     */
