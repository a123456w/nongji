...
public abstract class Utils {
     ...
     *
     * 根据演示文本计算文本的大致宽度。避免重复调用（例如内部绘图方法）
     *
     * @param paint
     * @param demoText
     * @return
    ...
    public static void drawXAxisValue(Canvas c, String text, float x, float y,
        ...
        // Android does not snap the bounds to line boundaries, Android没有将界限限制在线边界上，
        //  and draws from bottom to top.从底部到顶部绘制。
        // And we want to normalize it.我们想使它正常化。
        drawOffsetY += -mFontMetricsBuffer.ascent;

        // To have a consistent point of reference, we always draw left-aligned 为了获得一致的参考点，我们总是向左对齐。
        Paint.Align originalTextAlign = paint.getTextAlign();
        paint.setTextAlign(Paint.Align.LEFT);
            //angleDegrees 偏转角度 此地可解决偏转后文字尾部不与X轴对其的bug
        if (angleDegrees != 0.f) {
            /**
             * TextRotationCenterX 表示字体旋转中心的X 值为0为View的最左边 1为View的右边
             * TextRotationCenterY 表示字体旋转中心的Y 值为0为View的最上边 1为View的下边
             */
            float TextRotationCenterX=0.5f;
            float TextRotationCenterY=1f;

            // Move the text drawing rect in a way that it always rotates around its center 以始终围绕其中心旋转的方式移动文本绘制ReCt
            drawOffsetX -= mDrawTextRectBuffer.width() *TextRotationCenterX;
            drawOffsetY -= lineHeight *TextRotationCenterY;

            float translateX = x;
            float translateY = y;

            // Move the "outer" rect relative to the anchor, assuming its centered  移动“外部”RcT相对于锚，假设它的中心
