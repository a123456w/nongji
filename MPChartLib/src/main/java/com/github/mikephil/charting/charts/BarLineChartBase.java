...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends
    ...
    // /** the approximator object used for data filtering 用于数据过滤的近似对象 */
    // private Approximator mApproximator; 专用逼近器M逼近器；
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // make sure the data cannot be drawn outside the content-rect 确保数据不能在内容摘要之外绘制
        ...
        // if highlighting is enabled 如果启用了高亮显示
        if (valuesToHighlight())
            mRenderer.drawHighlighted(canvas, mIndicesToHighlight);

        // Removes clipping rectangle 移除剪辑矩形
     ...
     * 通过根据当前视图中的条目重新计算最小和最大Y值来执行轴的自动缩放
     */
