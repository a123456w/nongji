...
public abstract class AxisRenderer extends Renderer {
    ...
    /** base axis this axis renderer works with 这个轴渲染器工作的基轴 */
    protected AxisBase mAxis;

    /** transformer to transform values to screen pixels and return  变压器将值转换为屏幕像素和返回值*/
     ...
     * 计算该轴的值。
     *
     * @param min - the minimum value in the data object for this axis 此轴的数据对象中的最小值
     * @param max - the maximum value in the data object for this axis 此轴的数据对象中的最大值
     */
    public void computeAxis(float min, float max, boolean inverted) {

        // calculate the starting and entry point of the y-labels (depending on
        // zoom / contentrect bounds)计算Y标签的起始点和入口点（取决于缩放/ CordrRT边界）
    ...
    protected void computeAxisValues(float min, float max) {
        ...
        // Find out how much spacing (in y value space) between axis values 找出轴值之间的间距（y值空间）
...
//        double interval = Utils.roundToNextSignificant(rawInterval);
        ...
        if (mAxis.isForceLabelsEnabled()) {
            ...
            if (mAxis.mEntries.length < labelCount) {
                // Ensure stops contains at least numStops elements.确保停止至少包含numstops元素。
            ...
            if (mAxis.mEntries.length < n) {
                // Ensure stops contains at least numStops elements. 确保停止至少包含NUSTORT元素。
                mAxis.mEntries = new float[n];
            }

            for (f = first, i = 0; i < n; f += interval, ++i) {

                if (f == 0.0) // Fix for negative zero case (Where value == -0.0, and 0.0 == -0.0) 负零情况下的修正
                    f = 0.0;
