...
public class SLogrecordAdapter extends RecyclerViewAdapter<LogrecordBean> {
    public SLogrecordAdapter(RecyclerView recyclerView) {
        super(recyclerView, R.layout.item_slogfragment);
    }
    ...
    protected void fillData(ViewHolderHelper viewHolderHelper, int position, LogrecordBean model) {
        ...
        viewHolderHelper.setText(R.id.tv_robot,model.getSn());
        ...
        if ("".equals(model.getLast_time())){
            viewHolderHelper.setText(R.id.servetrim,"");
        }else {
